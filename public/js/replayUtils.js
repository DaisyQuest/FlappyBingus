...
export function buildReplayPayload(
  ...
  score,
  {
    maxTicks = CLIENT_REPLAY_LIMITS.maxTicks,
    maxActions = CLIENT_REPLAY_LIMITS.maxActionsPerTick,
    maxRng = CLIENT_REPLAY_LIMITS.maxRngTape,
    maxBytes = CLIENT_REPLAY_LIMITS.maxBytes
  } = {}
) {
  if (
    !run ||
  ...
  const byteLimit = Number(maxBytes) || CLIENT_REPLAY_LIMITS.maxBytes;
  return isReplayWithinLimits(payload, byteLimit) ? payload : null;
}

export function isReplayWithinLimits(replay, maxBytes = CLIENT_REPLAY_LIMITS.maxBytes) {
  if (!replay || typeof replay !== "object") return false;
  try {
    const text = JSON.stringify(replay);
    const encoder = typeof TextEncoder !== "undefined" ? new TextEncoder() : null;
    const byteLength = encoder ? encoder.encode(text).length : text.length;
...
export function hydrateReplayFromServer(replay, limits = CLIENT_REPLAY_LIMITS) {
  ...
  const byteLimit = Number(limits?.maxBytes) || CLIENT_REPLAY_LIMITS.maxBytes;
  if (!isReplayWithinLimits(replay, byteLimit)) return null;
