...
describe("replay recording helpers", () => {
  ...
  it("rejects replay data that exceeds the byte limit", () => {
    const run = {
      ended: true,
      seed: "oversized",
      rngTape: [0.1, 0.2, 0.3],
    ...
    expect(buildReplayPayload(run, 10, { maxBytes: 10 })).toBeNull();

    const payload = buildReplayPayload(run, 10, { maxBytes: 10_000 });
    expect(isReplayWithinLimits(payload, 10_000)).toBe(true);
    expect(isReplayWithinLimits(payload, 10)).toBe(false);
    expect(hydrateReplayFromServer(payload, { ...__replayLimits, maxBytes: 10 })).toBeNull();
  });
