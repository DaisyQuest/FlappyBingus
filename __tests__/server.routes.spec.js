...
describe("server routes and helpers", () => {
  ...
  it("serves the replay browser page", async () => {
    ...
    expect(res.body).not.toContain("adsbygoogle.js");
    expect(res.body).not.toContain("/js/adsenseLoader.js");
  });
  ...
  it("serves a highscores HTML page", async () => {
    const { server } = await importServer();
    const res = createRes();
  ...
  it("serves previews as JSON by default and HTML when requested", async () => {
    ...
    expect(htmlRes.body).not.toContain("adsbygoogle.js");
    expect(htmlRes.body).not.toContain("/js/adsenseLoader.js");
  });
  ...
  it("returns 404 for missing ads.txt", async () => {
    const { server } = await importServer();
    const res = createRes();
