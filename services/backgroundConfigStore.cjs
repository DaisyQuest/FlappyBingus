...
function resolveConfigPath(configPath) {
  if (!configPath) {
    throw new Error("background_config_path_required");
  }
  return path.resolve(configPath);
}

function createBackgroundConfigStore({ configPath, fsApi = fs, now = Date.now, persistence = null } = {}) {
